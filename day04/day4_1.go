...
func main() {
	...
	var longestSleeper, mins int
	for g, h := range data {
		if t := h.total(); t > mins {
			longestSleeper, mins = g, t
		}
	}
	var bestMinute int
	for m, v := range data[longestSleeper] {
		if v > data[longestSleeper][bestMinute] {
			bestMinute = m
		}
	}
	fmt.Printf("Guard %d during minute %d: %d\n", longestSleeper, bestMinute, longestSleeper*bestMinute)
}
